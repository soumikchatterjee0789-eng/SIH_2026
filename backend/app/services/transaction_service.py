...
Preview results are cached in-memory (per-process) keyed by a batch_token
so that "confirm=true" can re-use the already-validated rows without
re-parsing or trusting client-supplied data. This is a deliberately simple
approach appropriate for a hackathon MVP - in production this cache would
move to Redis or a short-lived DB table.
"""
...
# Simple in-memory preview cache: {batch_token: [validated_row_dict, ...]}
...
def parse_and_validate_csv(file_bytes: bytes) -> tuple[str, list[dict]]:
    try:
    ...
    _PREVIEW_CACHE[batch_token] = normalized_rows
    return batch_token, normalized_rows


def get_cached_preview(batch_token: str) -> list[dict]:
    rows = _PREVIEW_CACHE.get(batch_token)
    if rows is None:
        raise APIError(
            400,
            "This upload preview has expired or was never created. Please re-upload the CSV file.",
...
def confirm_and_store(db: Session, user_id: str, batch_token: str) -> tuple[int, int, str]:
    rows = get_cached_preview(batch_token)
    ...
    _PREVIEW_CACHE.pop(batch_token, None)
