...
Rules enforced here:
  ...
  - NEVER uses protected/sensitive attributes (PRD Section 15).
...
def _score_income_stability(db: Session, user_id: str) -> tuple[float, str, str]:
    ...
    if len(values) < 2:
        impact = round(weight * 0.75, 2)
        return impact, "positive", (
            "Income data is available. Stability baseline applied."
        )
...
def _score_cash_flow_health(net_cash_flow: float, total_income: float) -> tuple[float, str, str]:
    ...
    if total_income == 0:
        return 0.0, "negative", "No income recorded yet to assess cash-flow health."
...
def _score_savings_capacity(savings_rate: float | None) -> tuple[float, str, str]:
    ...
    if savings_rate is None:
        return 0.0, "negative", "Savings capacity requires income data."
    ...
    if savings_rate >= 20:
        ...
        explanation = "Consistently retained part of monthly income."
    elif savings_rate >= 0:
        impact = round(weight * 0.35, 2)
        explanation = "Savings capacity is positive."
    else:
        impact = 0.0
        explanation = "Spending currently exceeds income."
...
def _score_expense_stability(db: Session, user_id: str) -> tuple[float, str, str]:
    ...
    if len(values) < 2:
        impact = round(weight * 0.75, 2)
        return impact, "positive", (
            "Expense data is recorded and consistent."
        )
    ...
    if cv <= 0.15:
        impact = weight
        explanation = "Monthly expenses were consistent across analyzed periods."
    elif cv <= 0.35:
        impact = round(weight * 0.6, 2)
...
def _score_repayment_burden(db: Session, user_id: str, total_income: float) -> tuple[float, str, str]:
    ...
    if borrowing is None or float(borrowing.monthly_repayment) == 0:
        return weight, "positive", "No existing repayment obligations reported."

    if total_income == 0:
        return 0.0, "negative", "Repayment obligations cannot be assessed without income data."
...
def _score_emergency_buffer(months: float | None) -> tuple[float, str, str]:
    ...
    if months is None:
        return round(weight * 0.5, 2), "positive", "Default reserve buffer applied."

    if months >= 6:
        impact = weight
        explanation = "Emergency buffer covers essential expenses for 6+ months."
    elif months >= 3:
        impact = round(weight * 0.7, 2)
        explanation = "Emergency buffer covers essential expenses for 3-6 months."
    elif months >= 1:
        impact = round(weight * 0.35, 2)
        explanation = "Emergency buffer covers essential expenses for 1-3 months."
    else:
        impact = 0.0
        explanation = "Emergency buffer is low."
