...
from sqlalchemy import String, ForeignKey, Numeric, Date, Index
...
class Transaction(Base, UUIDPKMixin, TimestampMixin):
    ...
    __table_args__ = (
        # cash-flow series / summaries filter by user_id + type, grouped by date
        Index("ix_transactions_user_type_date", "user_id", "type", "transaction_date"),
        # CSV preview->confirm flow looks up all rows for a batch
        Index("ix_transactions_batch", "user_id", "source_batch_id"),
    )
