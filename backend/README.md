...
No advanced ML dependency is required — the credit-readiness and financial
health engines are transparent, deterministic, rule-based calculations
(see Section 6 below on why).
...
For production with PostgreSQL, replace this with a real migration tool
(Alembic) before going live — `create_all()` cannot handle schema changes
to existing tables.
...
dev database. 15/15 tests pass as of this build.
...
variable settings — never in code. Switch to Alembic migrations before your
...
later without breaking the existing contract: advanced ML anomaly
detection, the scenario simulator, voice/multilingual assistant, and
external financial-data integrations. The consent/explainability/
correctability workflow (the PRD's non-negotiable core) is fully implemented
and tested.
